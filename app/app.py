...
st.set_page_config(
    page_title="SafeSpeak | Toxic Content Detection",
    layout="wide",
...
# --- SIDEBAR / NAVIGATION ---
st.sidebar.markdown(
    '<div class="sidebar-brand">SafeSpeak API</div>', unsafe_allow_html=True
)
st.sidebar.markdown(
    "<div style='color: #8b949e; text-align: center; margin-bottom: 20px;'>Content Moderation Suite</div>",
    unsafe_allow_html=True,
)
...
if mode == "🏠 Home":
    ...
    with col1:
        ...
        <div class="feature-card">
            ...
            <div style="color: #8b949e;">Process user inputs in milliseconds. Built using highly optimized parallel decision trees (Random Forest) combined with sparse matrix text representations.</div>
        </div>
        """,
            unsafe_allow_html=True,
        )
...
# --- PAGE: COMMENT ANALYSIS ---
    ...
    st.markdown(
        "<p style='color: #8b949e;'>Test our moderation engine against single comments, chat messages, or short reviews.</p>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        comment = st.text_area(
            ...
            height=300,
            placeholder="Paste a user comment here to analyze...",
    ...
    with col2:
        ...
        if submit and comment.strip():
            ...
            try:
                resp = requests.post(
                    "http://localhost:8000/predict-comment", json={"text": comment}
                )
                end_time = time.time()
                if resp.status_code == 200:
                    ...
                    st.markdown(
                        f'<div class="main-classification" style="color: {title_color};">{main_class_text}</div>',
                        unsafe_allow_html=True,
                    )

                    max_prob = (
                        max(
                            [v for k, v in probs_dict.items() if k != "neutral"],
                            default=0.0,
                        )
                        if is_toxic
                        else probs_dict.get("neutral", 1.0)
                    )
                    ...
                    fig_gauge = go.Figure(
                        go.Indicator(
                            ...
                            number={
                                "font": {"color": title_color, "size": 50},
                                "suffix": "%",
                            },
                            gauge={
                                "axis": {
                    ...
                    if shap_data:
                        ...
                        st.markdown(
                            "<div class='shap-header' style='margin-top: 40px;'>🧠 Deep Model Explainability (SHAP)</div>",
                            unsafe_allow_html=True,
                        )
                        st.markdown(
                            "<p style='color: #aeb4b8; font-size: 14px; margin-bottom: 20px;'>Understanding the precise mathematical weight of each word that drove the AI's classification decision.</p>",
                            unsafe_allow_html=True,
                        )

                        # Colors based on impact direction
                        colors = [
                            "#f85149" if val > 0 else "#3fb950"
                    ...
                    for lbl in display_order:
                        ...
                        if lbl == "very_toxic":
                            disp_name = "Severe Toxic"
                        if lbl == "hate":
                            disp_name = "Hate Speech"

                        css_class = (
                            "bar-fill highlight" if prob_pct >= 50 else "bar-fill"
                        )
                        bars_html += f"""
...
# --- PAGE: POST ANALYSIS ---
    ...
    st.markdown(
        "<p style='color: #8b949e;'>Analyze engagement and content signals to predict visibility suppression.</p>",
        unsafe_allow_html=True,
    )
    ...
    with tab_manual:
        ...
        with col1:
            ...
            post_text = st.text_area(
                "Post Content", height=150, placeholder="Enter post content here..."
            )
            ...
            watch_time = st.slider(
                "Watch Time %", min_value=0.0, max_value=100.0, value=50.0, step=1.0
            )

            st.markdown("<br>", unsafe_allow_html=True)
            analyze_btn = st.button(
                "Analyze Post", type="primary", use_container_width=True
            )
            ...
            if analyze_btn:
                if not post_text.strip():
                    ...
                    with st.spinner("Analyzing behavioral and content metrics..."):
                        ...
                        try:
                            resp = requests.post(
                                "http://localhost:8000/predict-post", json=payload
                            )
                            if resp.status_code == 200:
                                data = resp.json()
                                ...
                                if shap_data:
                                    ...
                                    st.markdown(
                                        "<p style='color: #aeb4b8; font-size: 14px; margin-bottom: 20px;'>A mathematical breakdown of the content and engagement signals that shifted the AI's visibility decision.</p>",
                                        unsafe_allow_html=True,
                                    )
                            ...
                            st.error(
                                f"Failed to connect to suppression API. Please assure the background thread is running: {e}"
                            )
            else:
                st.markdown(
                    """
    ...
    with tab_image:
        ...
        with colimg1:
            ...
            st.markdown(
                "<p style='text-align: center; color: #8b949e; margin: 10px 0;'>— OR —</p>",
                unsafe_allow_html=True,
            )
            ...
            if preview_source is not None:
                st.image(
                    preview_source,
                    caption="Captured Post Screenshot",
        ...
        with colimg2:
            ...
            if analyze_img_btn and image_bytes is not None:
                with st.spinner(
                    "Running EasyOCR extraction and multi-modal analysis... This may take a moment."
                ):
                    import requests
                    ...
                    try:
                        ...
                        resp = requests.post(
                            "http://localhost:8000/predict-post-image", files=files
                        )
                        ...
                        if resp.status_code == 200:
                            ...
                            st.info(
                                f"**Hashtags:** {', '.join(extracted.get('hashtags', []))}"
                            )
                            ...
                            st.markdown(
                                f"""
                            <div style='background-color: rgba(15,15,20,0.5); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.05); border-radius: 12px; padding: 20px; margin-bottom: 20px; box-shadow: 0 4px 15px rgba(0,0,0,0.2);'>
                                <div style='color: #e0e0e0; font-weight: 600; font-size: 18px; margin-bottom: 12px; letter-spacing: 0.5px;'><span style="margin-right: 8px;"></span> SHAP Based AI Explanation </div>
                                <div style='color: #aeb4b8; font-size: 15px; line-height: 1.6;'>{summary_text}</div>
                            </div>
                            """,
                                unsafe_allow_html=True,
...
# --- PAGE: MODEL VISUALIZATIONS ---
elif mode == "📊 Model Visualizations":
    st.markdown("## 📊 Model Performance & Data Visualizations")
    st.markdown(
        "<p style='color: #8b949e;'>Explore the data distribution and training evaluation metrics generated by the learning pipeline.</p>",
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        try:
            ...
            st.warning("ROC plot missing.")
    with col2:
        st.markdown("### 📉 Precision-Recall Curves")
        try:
            st.image("plots/pr_curves.png", use_container_width=True)
        except:
            st.warning("PR Curves missing.")
    ...
    with col3:
        ...
        try:
            ...
            st.warning("Feature importance missing.")
    with col4:
        st.markdown("### 📑 Data Imbalance & Distribution")
        try:
            st.image("plots/class_imbalance.png", use_container_width=True)
            st.image("plots/word_count_distribution.png", use_container_width=True)
        except:
            st.warning("EDA plots missing.")
