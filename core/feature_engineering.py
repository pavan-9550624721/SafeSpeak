...
def extract_custom_features(texts):
    ...
    df_features["uppercase_ratio"] = texts.apply(
    ...
    df_features["sentiment_score"] = texts.apply(
        lambda x: TextBlob(str(x)).sentiment.polarity
    )
    df_features["profanity_count"] = texts.apply(
        lambda x: sum(1 for word in str(x).lower().split() if word in BANNED_WORDS)
    )
    df_features["unique_word_ratio"] = texts.apply(
...
def run_feature_engineering(csv_path):
    ...
    plt.close()
    print(
        "[INFO] Saved word count distribution plot to 'plots/word_count_distribution.png'"
    )
    ...
    vectorizer = TfidfVectorizer(
        max_features=20000, ngram_range=(1, 2), stop_words="english", lowercase=True
    )
