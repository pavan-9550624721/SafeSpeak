...
def train_toxicity():
    ...
    df.rename(
        columns={"severe_toxic": "very_toxic", "identity_hate": "hate"}, inplace=True
    )
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        df_sample["clean_text"], df_sample[target_cols], test_size=0.1, random_state=42
    )
...
def train_spam():
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        df["clean_text"], df["is_spam"], test_size=0.2, random_state=42
    )
