...
def predict_post(req: PostRequest):
    ...
    row_data = (
        ...
        + [spam_prob]
        + list(saf_probs)
        + [engagement_ratio, float(req.watch_time)]
    )
    ...
    rf_pred_idx = (
        list(base_rf.classes_).index(pred) if pred in base_rf.classes_ else pred_idx
    )
...
def predict_comment(req: CommentRequest):
    ...
    explainer = shap.LinearExplainer(
        (
            tox_model.estimators_[list(probs_dict.keys()).index(dominant_label)]
            if dominant_label != "neutral"
            else tox_model.estimators_[0]
        ),
        tox_vec.transform([""]),
    )
...
async def predict_post_image(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to process image: {safe_error_str}"
        )
